...
def placeOrder(userid):
    # try:
        ...
        try:
            ...
            print("ERRORRRR   Orderid in error:", orderid)
            print(query)
            ...
            try:
                for k in range(len(myorderitemsid)):
                    ...
                    try:
                        ...
                        print("ERROR Failed to insert in orderitem")

            except :
                print("Error in adding in order item")
            
            
            moneyTransactionManagement(userid, totalamt,myorderitemsid,demandedQuantity,selleridarray, price, availableQuantity,orderid)
    ...
    # except:
    #     print(" ERROR Aborting all changes ")
    #     conn.rollback()
            



def moneyTransactionManagement(userid, totalamt,myorderitemsid,demandedQuantity,selleridarray, price, availableQuantity,orderid):
    ...
    try:
        ...
        print("ERRRO Could not insert into paymentsmethod")
    ...
    for k in range(len(selleridarray)):
        ...
        cursor.execute(query) 
    try:
        query = f"insert into  product SET stockqty = {value} WHERE productid = {myorderitemsid[k]};"
        cursor.execute(query)
    except:
    ...
    query = f"UPDATE orders SET userid = {userid}, orderdate = '{current_timestamp}', totalamt = {totalamt}, shippingaddressid = 1 WHERE orderid = {orderid};"
    print(query)
    cursor.execute(query)
    # try:
    #     cursor.execute(query)
    ...
    # except :
    #     print(query)
    #     print("ERROR could not update order")
    print("Successfull")
